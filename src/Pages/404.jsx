...
export default function NotFoundPage() {
  useEffect(() => {
    // ✅ เมื่อไปหน้า 404 ให้เช็ค auth และ redirect ถ้า session หมด
    const checkAuthOn404 = () => {
      const authStatus = localStorage.getItem("AUTH_LOGIN");
      const expiryTime = localStorage.getItem("AUTH_EXPIRY_TIME");
      const currentTime = Date.now();
      
      if (!authStatus || !expiryTime || currentTime > parseInt(expiryTime)) {
        console.log("⏰ Session expired on 404 page - Redirecting to login");
